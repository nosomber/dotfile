...
update() {
    ...
    elif [ "$vol_text" -eq 0 ];  then vol_text="mute"; vol_icon="";
...
notify() {
    update
    vol_num="$vol_text "
    if [ $vol_num == "mute" ]; then
        vol_num="00"
    fi
    notify-send -r 9527 -h int:value:$vol_num -h string:hlcolor:#dddddd "$vol_icon Volume"
}
...
click() {
    case "$1" in
        ...
        U) pactl set-sink-volume @DEFAULT_SINK@ +2%; notify ;; # 音量加
        D) pactl set-sink-volume @DEFAULT_SINK@ -2%; notify ;; # 音量减
    esac
}
...
case "$1" in
    click) click $2 ;;
    notify) notify ;;
    up)  ~/scripts/sys/volume.sh raise; notify ;; 
    down)  ~/scripts/sys/volume.sh lower; notify ;; 
    mute)  ~/scripts/sys/volume.sh mute; notify ;; 
    *) update ;;
esac

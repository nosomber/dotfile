...
update() {
    wifi_icon=""
    wifi_text=$(nmcli | grep "$wifi_grep_keyword" | awk -F "$wifi_grep_keyword" '{print $2}' | awk '{print substr($0, 2)}')
    [ "$wifi_text" = "" ] && wifi_text=$wifi_disconnected

    icon=" $wifi_icon"
		if [ $(nmcli | grep "$wifi_grep_keyword" | awk -F "$wifi_grep_keyword" '{print $2}' | awk 'END{print NR}') -eq "2" ]; then
				text="XoX |"
		elif [ "$wifi_text" != "Wired connection 1" ]; then
				text="$wifi_text "
		else
				text="wired "
		fi

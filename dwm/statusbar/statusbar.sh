...
# 更新状态栏
refresh() {
    _wifi='';_date='';_vol='';_bat='' #重置所有模块的状态为空
    source $tempfile                                                     # 从 temp 文件中读取模块的状态
    xsetroot -name "$_wifi$_date$_vol$_bat"      # 更新状态栏
}
...
cron() {
    ...
    while true; do
        ...
        [ $((i % 20)) -eq 0 ]  && to=(${to[@]} vol)                      # 每 20秒  更新 cpu mem vol 
        [ $((i % 300)) -eq 0 ] && to=(${to[@]} bat)                      # 每 300秒 更新 bat
        [ $((i % 5)) -eq 0 ]   && to=(${to[@]} date)                     # 每 5秒   更新 date
        [ $i -lt 30 ] && to=(wifi date vol bat)                          # 前 30秒  更新所有模块
...
case $1 in
    ...
    updateall|check) update wifi date vol bat ;;
